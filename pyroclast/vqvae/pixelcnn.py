...
def wn_linear(in_dim, out_dim):
    return WeightNorm(snt.Linear(in_dim, out_dim))


class WNConv2d(tf.Module):
...
def shift_down(input, size=1):
    return tf.pad(input, [[size, 0], [0, 0]])[:, :input.shape[1], :, :]
...
class CausalConv2d(tf.Module):
    ...
    def __init__(
        ...
        self.pad = lambda x: tf.pad(x, pad)
...
class CondResNet(tf.Module):

    def __init__(self, in_channel, channel, kernel_size, n_res_block):
        super().__init__()

        blocks = [
            WNConv2d(in_channel, channel, kernel_size, padding=kernel_size // 2)
        ]

        for i in range(n_res_block):
            blocks.append(GatedResBlock(channel, channel, kernel_size))

        self.blocks = snt.Sequential(*blocks)
...
class PixelSNAIL(tf.Module):
    ...
    def __init__(
        ...
        causal_padding = lambda kernel, pad_idx=-1: snt.pad.create(
            snt.pad.causal, kernel, rate=1, n=2, channel_index=pad_idx)
        self.horizontal = snt.Conv2D(channels, [kernel // 2, kernel],
                                     padding=causal_padding(
                                         [kernel // 2, kernel]))
        self.vertical = CausalConv2d(channels, [(kernel + 1) // 2, kernel // 2],
                                     padding='downright')

        coord_x = (tf.cast(tf.range(height), tf.float32) - height / 2) / height
        coord_x = tf.tile(tf.reshape(coord_x, [1, 1, height, 1]),
                          [1, 1, 1, width])
        coord_y = (tf.cast(tf.range(width), tf.float32) - width / 2) / width
        coord_y = tf.tile(tf.reshape(coord_y, [1, 1, 1, width]),
                          [1, 1, height, 1])
        ...
        out.extend([tf.nn.elu, WNConv2d(channel, n_class, 1)])
