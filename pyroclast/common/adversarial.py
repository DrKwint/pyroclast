...
def fast_gradient_sign_method(feature_fn, classify_fn, x, y, eps, norm):
    ...
    Returns:
        perturbed_data (Tensor): adversarially perturbed data to minimize \
    ...
    return tf.expand_dims(tf.expand_dims(x, [1]), [1]) + perturbation
...
def linear_optimization(jacobian, eps, norm):
    ...
    Returns:
        scaled_perturbation (Tensor): perturbation of shape [N,F,Cl,...] which \
            maximizes the use of jacobian w.r.t. scaled to eps limit under norm
    """
